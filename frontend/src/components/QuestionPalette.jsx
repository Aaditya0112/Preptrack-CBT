...
export default function QuestionPalette() {
  ...
  const assessmentType = state?.assessmentType || 'exam';
  ...
  return (
    <div className=" flex flex-col grow bg-white shadow-md overflow-y-auto">
      ...
      <div className='border flex-1 overflow-auto border-black bg-sky-100 max-h-full'>
          ...
          <div key={currentSection.id}>
            ...
            <div className="grid grid-cols-5 gap-2 p-3 rounded-b-lg">
              {state.questions.map((q, index) => {
                if (q.sectionId === currentSection.id || q.practice) {
        ...
        {/* TODO HANDEL TYPE OF TEST IN STATE */}
        {state && assessmentType === 'practice' && (
          <div key={'all-questions'}>
            <h5 className="font-semibold p-2 bg-blue-400 text-white">All Questions</h5>
            <h3 className='p-2'>Choose a Question</h3>
            <div className="grid grid-cols-5 gap-2 p-3 rounded-b-lg">
              {state.questions.map((q, index) => {
                if ( q.practice) {
